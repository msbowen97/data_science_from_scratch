...
class Model:
    ...
    def _calc_sigmoid(self, yHat):
        ...
        '''
        return 1 / (1 + np.exp(np.clip(-yHat, -30, 30)))
...
class LossFunction:
    ...
    def __call__(self, X, y, yHat, calcGrad=True, calcHess=False):
        '''
        ...
        self.hess = self.get_hess(X, y) if calcHess else 0
    ...
    #     if self.modelMethod == 'logreg' and self.lossMethod == 'binary_cross_entropy':
    #         W = self.yHat * (1 - self.yHat)
    #         return X.T @ (W[:, None] * X) / n 
    ...
    def _check_convergence(self, epoch, grad=[np.inf], losses=[np.inf]):
        ...
        if epoch >= self.minEpochs:
            if np.linalg.norm(grad) < self.gradThreshold:
                ...
                return True
            elif np.linalg.norm(np.abs(np.diff(losses[-self.lossWindow:]))) <= self.lossThreshold and len(losses) > self.lossWindow:
...
class GradientDescent(OptimizerPrototype):
    ...
    def __init__(self, optimizerMethod, lr=0.1, lrMin=0.01, lrDecay=-1e-4, lrUpdate='percent', 
                 initialGuess='zeros', maxEpochs=1e4, minEpochs=10, gradThreshold=1e-5, lossThreshold=-1, lossWindow=100,
    ...
    def train(self):
        ...
        while True:
            ...
            self.losses.append(epochLoss.copy() / self.epochUpdate)
            self.epoch.update()
...
class SimulatedAnnealing(OptimizerPrototype):
    ...
    def train(self):
        ...
        self.bestLoss = self.lossFunc.loss
...
class KNN(OptimizerPrototype):
    # To-do: Adaptive-k, prior class/cost-sensitive knn, 
    ...
    def __call__(self, X, y, model=None, lossFunc=None):
        ...
        if self.distanceMethod == 'mahalanobis':
            try:
                cov_matrix = np.cov(self.X.T)
                self.inv_cov = np.linalg.inv(cov_matrix)
            except:
                print('Unable to calculate covariance matrix. Switching to euclidean distance.')
                self.distanceMethod == 'euclidean'

    def _calc_distance(self, x):
        if self.distanceMethod == 'euclidean':
            return np.sum((self.X - x) ** 2, axis=1)
        elif self.distanceMethod == 'mahalanobis':
            delta = self.X - x
            return np.einsum('ij,jk,ik->i', delta, self.inv_cov, delta)
        else:
            print('Invalid distanceMethod provided. Defaulting to Euclidean')
            return np.sum((self.X - x) ** 2, axis=1)
